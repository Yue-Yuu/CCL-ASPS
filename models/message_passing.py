...
class MessagePassing(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.combine_linear = torch.nn.Linear(self.out_channels*2, self.out_channels)
        temp = 0
    ...
    def propagate(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        if is_sparse(edge_index) and self.fuse and not self.explain:
            ...
            for i in range(decomposed_layers):
                ...
                # x_i = kwargs['x'][0][coll_dict['edge_index_i']].unsqueeze(2)
                # x_j = coll_dict['x_j'].unsqueeze(2)
                # # x_pair = torch.cat((x_j,x_i),dim=2)
                # # x_i = kwargs['x'][0][coll_dict['edge_index_i']]
                # # x_j = coll_dict['x_j']
                # x_pair = torch.cat((x_j, x_i), dim=-1)
                # x_j = x_pair.view(-1,2,x_j.shape[-1])
                # # x_j = self.combine_Conv1d(x_j)
                # # x_j = x_j.view(x_pair.shape[0],x_pair.shape[1],-1)
                # # x_j = self.combine_pooling(x_j)
                # # x_j = x_j.view(x_pair.shape[0],-1)
                # # x_j = x_j.view(x_pair.shape[0] * x_pair.shape[1], -1)
                # x_j = self.combine_linear(x_pair)
                ...
                """BIONIC在此处更改message函数
                   使用带初始权重的softmax函数更新特征
                """
        ...
        """
        卷积操作改变了节点本身的特征，重新添加节点本身的特征
        """
        # out = (out + kwargs['x'][1]) / 2
